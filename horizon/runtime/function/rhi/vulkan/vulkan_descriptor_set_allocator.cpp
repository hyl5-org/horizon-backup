...
void VulkanDescriptorSetAllocator::CreateDescriptorSetLayout(VulkanPipeline *pipeline) {
    ...
    //    auto bindings = pipeline->GetDescriptorSetLayoutBinding(static_cast<ResourceUpdateFrequency>(i));
    //    if (bindings.empty()) {
    //        continue;
    //    }
    //    layout_create_infos[i].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    //    layout_create_infos[i].pNext = nullptr;
    //    layout_create_infos[i].flags;
    //    layout_create_infos[i].bindingCount = bindings.size();
    //    layout_create_infos[i].pBindings = bindings.data();
    ...
    //        VkDescriptorSetLayout layout;
    //        CHECK_VK_RESULT(vkCreateDescriptorSetLayout(m_context.device, &layout_create_infos[i], nullptr, &layout));
