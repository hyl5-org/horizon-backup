...
void HorizonPipeline::run() {
    ...
    while (!engine->m_window->ShouldClose()) {
        ...
        auto geometry_pass_per_frame_ds = rhi->GetDescriptorSet(geometry->geometry_pass,ResourceUpdateFrequency::PER_FRAME);
        ...
        auto geometry_pass_bindless_ds = rhi->GetDescriptorSet(geometry->geometry_pass, ResourceUpdateFrequency::BINDLESS);
        ...
        geometry_pass_bindless_ds->SetResource(material_textures, "material_textures");
        ...
        auto gp_semaphore = rhi->CreateSemaphore1();
        {
            ...
            // deferred decal
            {
                //auto decal_ds =  rhi->GetDescriptorSet(decal->decal_pass, ResourceUpdateFrequency::PER_FRAME);
                ...
                //auto decal_pass_bindless_ds = rhi->GetDescriptorSet(decal->decal_pass, ResourceUpdateFrequency::BINDLESS);
                ...
                //decal_pass_bindless_ds->SetResource(decal_material_textures, "decal_material_textures");
        ...
        // all following pass use compute queue
        {
            ...
            auto ssao_ds = rhi->GetDescriptorSet(ssao->ssao_pass, ResourceUpdateFrequency::PER_FRAME);
            // ao pass
            {
            ...
            auto ao_blur_ds = rhi->GetDescriptorSet(ssao->ssao_blur_pass, ResourceUpdateFrequency::PER_FRAME);
            ...
            // shading pass
            {
                auto shading_ds = rhi->GetDescriptorSet(shading->shading_pass, ResourceUpdateFrequency::PER_FRAME);
            ...
            auto pp_ds = rhi->GetDescriptorSet(post_process->post_process_pass, ResourceUpdateFrequency::PER_FRAME);
            {
...

            {
                ...
                auto taa_ds = rhi->GetDescriptorSet(antialiasing->taa_pass, ResourceUpdateFrequency::PER_FRAME);
                {
